...
export type BlockOverlayProps = {
  ...
  extraRects?: Rect[];
  /** The trade button's own card (`hostBox`): the block is flush with it and never leaves it. */
  containerRect?: Rect;
  kind: TargetKind;
  hits: HitDto[];
  phrase: string;
 ...
 */
export function BlockOverlay({ anchorRect, extraRects, containerRect, kind, hits, phrase, onEvidence, onOverride, pending = false, error = null, replay, venue }: BlockOverlayProps) {
  const frameRef = useRef<HTMLDivElement>(null);
  const [contentHeight, setContentHeight] = useState(0);
  const [, remeasure] = useReducer((n: number) => n + 1, 0);
  const rect = computeBlockRect(anchorRect, undefined, contentHeight, { width: window.innerWidth }, extraRects, containerRect);
