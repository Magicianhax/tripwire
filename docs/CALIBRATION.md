...
### 8.4 What Round 2.2 added to the open list (2026-09-20)

Round 2.2 widened `smart_side_disagrees` from Yes/No markets to **any market's own outcome set**. Under §8.3 this is a data change: the 50 / 70 boundaries, the `null` → UNCHECKED rule and `presets.ts` are all untouched, and a Yes/No market computes byte-for-byte what it computed before (`packages/core/test/prediction-outcomes.test.ts`, "a Yes/No market computes exactly what it computed before"). What it does change is **which markets produce a value at all**, and that is new population, not a new threshold.

Measured on Polymarket's live Gamma API on 2026-09-20 (0 Nansen credits): **44 of the 100 highest-24h-volume open markets have outcomes other than Yes/No** — NFL and esports moneylines, spreads, totals. Every one of them was `null` / UNCHECKED before this round and can now fire a rule.

1. **The 50 / 70 boundaries have never been seen against a sports book.** Every number behind them came from Yes/No markets. A moneyline's holder base, its position sizes and its proven-winner concentration are not obviously the same distribution, and the population the thresholds now apply to is roughly 1.8× the one they were derived on. The 8.2.1 measurement run must sample **both** kinds and report their distributions separately before either boundary moves; until then a sports market is judged by a Yes/No calibration, which is stated here rather than discovered later.
2. **`unmatchedUsd` is reported and never scored.** Proven-winner money whose `side` is in none of the market's outcomes is kept out of both halves of the ratio and shown on the card as its own line. How often it is non-zero on a live sports market is unmeasured; if it turns out to be common, the ratio's denominator is smaller than the sample and that is a calibration question, not a render one.
3. **Markets with more than two outcomes are unmeasured and currently unobserved.** None of the top 100 had three or more. The signal sums every non-picked outcome into one "opposing" figure, which is the only honest generalisation of "the other side" but has no measurement behind it. If a three-way market ever reaches the top of the book, treat its value as uncalibrated.
4. **The picker cannot move a verdict, by construction.** An ambiguous event stays UNCHECKED with `market: null`, and the event picker is evidence: nothing it renders is fetched per row and nothing it renders is scored. There is no calibration surface here, only the note that a user who opens a sibling market gets that market's own card with its own verdict.
