...
Optional: `NANSEN_PER_IP_DAILY_CREDITS` (default 6000: every install minted from one network
shares this), `TRIPWIRE_MINTS_PER_IP` (default 20/day), `TRIPWIRE_MINTS_PER_DAY` (default
2000/day). Networks are identified only by `Fly-Client-IP`, which Fly's edge sets, and stored
only as a salted hash; a hosted mint with no client address is refused. Spend is bounded per
network rather than by how many installs it mints, so a household or office is not locked out
and no single address can use more than 6000 of the 20000 global credits in a day.
